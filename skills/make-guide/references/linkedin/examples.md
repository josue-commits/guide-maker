...
**Do not edit this file; a skill update replaces it.** These three are synthetic, written to show the house structure on generic topics. They are voice and shape references, not proof of performance. Put 4-8 of your own posts that worked in `<project>/.guide-maker/top-performers.md` and the writer agent matches those first. To replace these examples outright, write `<project>/.guide-maker/examples.md`; the writer reads that file instead of this one.
